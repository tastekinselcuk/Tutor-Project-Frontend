<template>
  ...
  <nav id="navbar" class="navbar navbar-expand-lg navbar-light">
    <div class="container-fluid">
      <a id="logo" class="navbar-brand ml-30" href="#" @click.prevent="handleLogoClick">
        <img class="navbar-brand" src="../assets/images/Logo/logo2.png" alt="TUTORDAN" />
      </a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav-1">
        ...
        <div id="login" class="navbar-nav" v-if="!isAuthenticated">
          <router-link class="auth-dependant login-button" to="/login">
            Giriş Yap<ion-icon name="log-in-outline"></ion-icon>
          </router-link>
        </div>
        <!--Profile dropdown-->
        <div class="dropdown auth-dependant login-button" v-if="isAuthenticated">
  ...
  methods: {
    handleLogoClick() {
      if (!this.isAuthenticated) {
        this.$router.push('/');
      } else if (this.getCurrentUser.role === 'STUDENT') {
        this.$router.push('/studentDashboard');
      } else if (this.getCurrentUser.role === 'TUTOR') {
